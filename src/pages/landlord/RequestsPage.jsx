...
export default function RequestsPage() {
  ...
  const handleConvert = async (r) => {
    if (!window.confirm(
      r.tenant
        ...
        : `Create a tenant account for "${r.lead_name || r.tenant_name || 'this applicant'}"? They'll be able to log in and manage their own applications.`
    )) return
    setConverting(true)
    try {
  ...
  return (
    <div className="p-6 animate-fade-up">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {r.tenant ? (
                    ...
                    <ActionBtn variant="amber" onClick={() => handleConvert(r)} disabled={converting}>
                      <i className="bi bi-person-plus-fill mr-1"></i>Register as Tenant
                    </ActionBtn>
                  )}
                  {r.status === 'PENDING' && (
                    <>
      ...
      {convertedInfo && (
        <Modal title="Tenant Account Ready" onClose={() => setConvertedInfo(null)}>
          <div className="space-y-4">
            ...
            {convertedInfo.reused_existing ? (
              <div className="text-sm text-gray-600 bg-gray-50 rounded-xl p-4">
                ...
                They keep their existing email &amp; password — no new login needed.
              </div>
            ) : (
